...
static void dilate(Vec3b Color,int n,Mat &img , Mat &img2)
{
    int & rows = img.rows;
    int & cols = img.cols;
    for(int row = n;row<rows-n;row++){
...
void ShowGridMap(shared_ptr<Maps::GridMap> ptr){
    ...
    for(int x=0;x<ptr->getSizeX();x++) for(int y=0;y<ptr->getSizeY();y++){
...
void EventMouseClick(int event, int x, int y, int flags, void* ustc)
{
    ...
    if(event == CV_EVENT_RBUTTONDOWN){
        ClearImage();
        ptr3 = ptr2;
        ptr3->Dilate(ptr2,Planning::Searcher::HybridAStarGlobal::DilateKernal);
        ShowGridMap(ptr3);
    }
    // if (event == CV_EVENT_RBUTTONDOWN)
    // {
